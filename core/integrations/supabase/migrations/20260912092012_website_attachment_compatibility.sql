...
begin
  ...
  if found then
    ...
    return jsonb_build_object('ok',true,'id',v_existing.id,'nr',v_existing.anfragenummer,'doppelt',true,
      'fotos',(select count(*) from public.crm_fotos where anfrage_id=v_existing.id));
  end if;
  ...
  if v_kunde is null then
    ...
    v_typ:=case p_payload->>'kundentyp' when 'Hausverwaltung / Eigentümer' then 'verwaltung'::public.kunden_typ
      when 'Generalunternehmer' then 'generalunternehmer'::public.kunden_typ when 'Gewerbe / Büro / Laden' then 'firma'::public.kunden_typ else 'privat'::public.kunden_typ end;
  ...
  for v_foto in select value from jsonb_array_elements(v_fotos) loop
    -- Website-Anlagen gehören zum bestehenden CRM-Kanal (nicht zum Subunternehmer-Portal).
    insert into public.crm_fotos(anfrage_id,phase,titel,daten,mini,bytes,sortierung,quelle,notiz)
    values(v_id,case when v_foto->>'daten' like 'data:application/pdf;%' then 'dokument' else 'vorher' end,
      left(coalesce(v_foto->>'titel','Anlage'),120),v_foto->>'daten',
      case when length(v_foto->>'mini')<80000 and v_foto->>'mini' ~ '^data:image/jpeg;base64,[A-Za-z0-9+/=]+$' then v_foto->>'mini'
        when v_foto->>'daten' like 'data:image/%' then v_foto->>'daten' else '' end,
      octet_length(decode(split_part(v_foto->>'daten',',',2),'base64')),v_i,'crm','Website-Anlage');
    v_i:=v_i+1;
  end loop;
