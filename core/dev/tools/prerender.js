...
const ORIGIN='https://www.baustern.ch';
...
function fill(tpl,d){let h=tpl;
  ...
  set(/<link rel="canonical" href="[^"]*">/,`<link rel="canonical" href="${d.url}">`);
...
(async()=>{
  ...
  for(const route of routes){
    ...
    out.push({route,file,url:d.url,title:d.title,bytes:fs.statSync(abs).size,h1:d.h1.trim().slice(0,50)});
  ...
  const rd=['# Generiert von tools/prerender.js — nicht von Hand editieren (Regeln oben ergänzen, unten bleibt der SPA-Fallback)',
