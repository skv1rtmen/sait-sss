...
## 21 · Этап 12 — страницы по городам и «Badsanierung × город» (18.09)

Владелец выбрал осторожный вариант: шесть новых городов плюс четыре страницы под связку
«Badsanierung × город». Матрицу 12 × 3 сознательно не делали — это doorway-паттерн.

### 21.1 Первая версия была спамом. Замер это показал

Сгенерировав страницы по шаблону (менялись название города, время в пути и одна строка фокуса),
я измерил попарное сходство видимого текста:

| | страниц | символов | сходство (max) |
|---|---|---|---|
| города, 1-я версия | 9 | ~2 950 | **0,986** |
| Badsanierung, 1-я версия | 4 | ~4 690 | **0,990** |

96–99 % — ровно тот случай, за который Google режет как за doorway-страницы. Отдавать в таком виде
нельзя было, поэтому содержание переписано.

### 21.2 Что сделало страницы разными

Для каждого города в `REGION_SEO` добавлены три собственных поля, и все они опираются на реальные
факты компании, а не на выдуманную местную специфику:

- `absatz` — четыре-пять предложений о том, чем работа именно в этом городе отличается: Дитикон —
  самая короткая дорога, поэтому окупается заказ от трёх часов; Баден — другой кантон, одна
  стройбригада на оба; Хорген — работа в заселённом доме, поэтому защита лестницы и вывоз мусора
  ежедневно; Люцерн — пятьдесят минут, поэтому работаем блоками и честно говорим, что мелкий ремонт
  туда брать не стоит.
- `svc` — три самых уместных гевёрка вместо одинакового блока из шести.
- `faq` — два своих вопроса на город.
- `bewilligungHinweis(kt,n)` — разный текст для Цюриха и для других кантонов (AG/ZG/LU).

Для четырёх страниц Badsanierung — свой разворот (`titel`, `absatz`, два своих вопроса):
логистика городской квартиры в Цюрихе, привязка к смене жильца в Винтертуре, серия одинаковых
ванных в Цуге, работа блоками из-за расстояния в Люцерне. Общая часть — шесть шагов санации,
ориентир цены из `RICHTWERTE` (CHF 9 000–26 000 работы, материал отдельно) и референс-проект.

Результат замера после переписывания:

| | сходство (max) | сходство (min) | символов |
|---|---|---|---|
| города (9) | **0,650** | 0,385 | ~3 560 |
| Badsanierung (4) | **0,825** | 0,770 | ~5 200 |

0,65 и 0,82 — это уже нормальный уровень для сайта с общей шапкой, подвалом и CTA.

### 21.3 Техника

- Новые маршруты регистрируются **из данных**: `REGION_SEO` и `GEWERK_SEO` порождают записи в
  `META` (pages.js) и в `ROUTES` (app.js). Новый город = одна запись в data.js, больше нигде.
- Внутренняя перелинковка: подвал перечисляет все девять городов, каждая городская страница ведёт
  на остальные и на свою страницу Badsanierung, страница Badsanierung — обратно на город.
  Без этого новые страницы остались бы сиротами и не ранжировались бы.
- Prerender: 31 → **41 маршрут**, sitemap и `_redirects` пересобраны.
- Порядок заголовков: блок `.promises` шёл сразу после `h1`, axe ругался на heading-order.
  Добавлены `h2` на обеих шаблонах.

### 21.4 Проверка

| | performance | accessibility | best-practices | SEO |
|---|---|---|---|---|
| `/badsanierung-zuerich` | 94–98 | **100** | 100 | 100 |
| `/sanierung-baden` | 96–98 | **100** | 100 | 100 |
| `/sanierung-winterthur` | 96 | **100** | 100 | 100 |

`pw-stage7.js` (телефон + десктоп, список маршрутов расширен новыми страницами) — **ALLES GRÜN**,
ноль ошибок JS.
